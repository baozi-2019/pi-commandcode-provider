...
要求 Node.js ≥ 22（与官方 `command-code` CLI 一致）。`@earendil-works/pi-ai` 与 `@earendil-works/pi-coding-agent` 是宿主 pi 打包提供的可选 peer 包，本仓库**不安装**它们——`tests/test-package-manifest.ts` 会校验它们没有出现在 `dependencies`/`devDependencies` 中。入口 `index.ts` 因此也不在 `tsconfig.json` 的 include 里（宿主加载期解析 peer 包），`npm run typecheck` 只覆盖 `src/` 与 `tests/`。
