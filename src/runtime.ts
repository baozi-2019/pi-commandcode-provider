...
export interface CommandCodeRuntimeOptions<TProviderConfig> {
  ...
  prepareModels?: (models: readonly CommandCodeModel[]) => {
...
export class CommandCodeRuntime<TProviderConfig, TContext extends CommandCodeCommandContext> {
  ...
  async initialize(): Promise<void> {
    ...
    const prepared = this.prepareModels(cached)
    this.pi.registerProvider("commandcode", this.options.createProviderConfig(prepared.models))
    this.providerRegistered = true
    this.status = {
      ...this.status,
      source: "cache",
      modelCount: prepared.models.length,
      catalogModelCount: cached.length,
      filteredModelCount: cached.length - prepared.models.length,
  ...
  refresh(force = false): Promise<CommandCodeRefreshResult> {
    if (this.refreshPromise) return this.refreshPromise

    const refreshPromise = this.refreshCatalog(force).finally(() => {
  ...
  private prepareModels(models: readonly CommandCodeModel[]): {
    models: readonly CommandCodeModel[]
    warning?: string
  } {
    return this.options.prepareModels?.(models) ?? { models }
  }

  private async refreshCatalog(force = false): Promise<CommandCodeRefreshResult> {
    this.status = {
      ...this.status,
    ...
    try {
      ...
      if (shouldRegister) {
        const prepared = this.prepareModels(loaded.models)
        this.pi.registerProvider("commandcode", this.options.createProviderConfig(prepared.models))
        this.providerRegistered = true
        ...
        if (prepared.models.length === 0) {
          ...
          this.status = {
            ...
            modelCount: 0,
            catalogModelCount: loaded.models.length,
            filteredModelCount: loaded.models.length,
            warning: prepared.warning ?? preservedWarning,
            refreshing: false,
          }
        ...
        this.status = {
          ...
          modelCount: prepared.models.length,
          catalogModelCount: loaded.models.length,
          filteredModelCount: loaded.models.length - prepared.models.length,
          lastSuccess: this.now(),
          warning: [prepared.warning, warning].filter(Boolean).join(" ") || undefined,
          refreshing: false,
        }
        const combinedWarning = [prepared.warning, warning].filter(Boolean).join(" ") || undefined
        if (combinedWarning) this.warn(combinedWarning)
        return {
          refreshed: true,
          source: loaded.source,
          modelCount: prepared.models.length,
