 ...
 *
 * `src/commandcode-catalog.ts` is a snapshot of the CLI package catalog; keep
 * it byte-identical to the generation output. Entries here are merged over the
 * generated catalog at load time. When upstream publishes its own levels for a
 * model listed here, drop that entry in the same catalog refresh so nothing has
